...
[x_initial, y_initial, depth] = current_coordinates(yellowMask);
...
if depth > 43
    ...
    err = setting_pose(arb, x_initial, y_initial, 8);
    if err == 1, return; end
    
    % Going to grasp position (z=6) CASE 1
    err = setting_pose(arb, x_initial, y_initial, 6);
    if err == 1, return; end
...
if depth < 43
    ...
    % Going to pre-grasp position (z = 8)
    err = setting_pose(arb, x_initial, y_initial, 10)
    if err == 1, return; end
...
% -------------------- CASE 1 ---------------------------------------------
if depth > 43
    err = setting_pose(arb, x_final, y_final, 10) % keeping it higher to prevent collision
    if err == 1, return; end
    
    % Going to place position.
    err = setting_pose(arb, x_final, y_final, 6)
    if err == 1, return; end
end
...
if depth < 43
    ...
    % Going to place position
    err = setting_pose(arb, x_final, y_final, 8)
    if err == 1, return; end
